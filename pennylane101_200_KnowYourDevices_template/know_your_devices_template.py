...
def compare_circuits(num_wires, params):
    ...
    def pure_circuit():
        ...
        #qml.broadcast(qml.RY, wires=num_wires, pattern='single', parameters=[p in params[0]])
            qml.RY(params[0][p],wires=p)
    ...
    def mixed_circuit():
        ...
        # create the circuit here
        for p in range(num_wires):
        #qml.broadcast(qml.RY, wires=num_wires, pattern='single', parameters=[params[0]])
            qml.RY(params[1][p],wires=p)
